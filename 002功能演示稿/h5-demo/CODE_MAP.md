...
| 初始布局渲染（优先默认样式） | [js/benefit-config.js](js/benefit-config.js) → `renderInitialLayout()` |
| 组件库渲染（三分组：装修/联盟/第三方） | [js/benefit-config.js](js/benefit-config.js) → `renderCompLibrary()` / `renderCompLibSections()` / `buildCompItemHTML()` |
| 组件库分组折叠 | [js/benefit-config.js](js/benefit-config.js) → `bindGroupCollapse()` |
| 组件库拖拽 | [js/benefit-config.js](js/benefit-config.js) → `bindCompLibDragEvents()`（跳过 .no-link 置灰项） |
...
| 缺失活动编辑弹窗 | [js/benefit-config.js](js/benefit-config.js) → `openMissingActModal()` / `saveMissingActPaths()`（已废弃，保存时不再校验缺失路径） |
...
| 保存/脏状态 | [js/benefit-config.js](js/benefit-config.js) → `saveAllConfig()` / `markDirty()` / `updateSaveButton()`（已移除保存前缺失路径校验） |
...
| Mock数据 - 装修组件模板 | [js/mock-data.js](js/mock-data.js) → `decorationTemplates[]`（标题组件，开发预编译） |
| Mock数据 - 默认样式 | [js/mock-data.js](js/mock-data.js) → `defaultStyleConfig`（新商户注册时自动应用的画布布局） |
