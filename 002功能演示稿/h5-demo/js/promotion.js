...
(function () {
  ...
  function renderPromoList() {
    ...
    if (list.length === 0) {
      tbody.innerHTML = '<tr><td colspan="8"><div class="empty-state"><div class="empty-state-icon">📌</div><div class="empty-state-text">暂无推广位数据</div></div></td></tr>';
    ...
    tbody.innerHTML = pageItems.map(function (p) {
      ...
      // 缺失活动
      var missingCell, h5PathCell, mpCell;
      ...
      mpCell =
        '<div class="promo-path-input-row" style="justify-content:center;">' +
        (mpCount > 0
  ...
  // ===== 第三方活动活动是否允许独立编辑 =====
  function isIndividualEdit(act) {
    return act.allowIndividualEdit !== undefined ? act.allowIndividualEdit : false;
  }
  ...
  function renderDrawerPathTable(promoId) {
    ...
    tbody.innerHTML = thirdActs.map(function (act) {
      var individual = isIndividualEdit(act);
      ...
      var hasFallback = isEmpty && individual && defaultPath;

      var clsSuffix, statusHtml, inputAttrs;
      if (!individual) {
        clsSuffix = ' is-unified';
        statusHtml = '<span class="path-drawer-status is-unified">统一</span>';
        inputAttrs = ' disabled title="未开启独立编辑，请到活动详情页修改"';
      } else if (hasFallback) {
        clsSuffix = ' is-default';
        statusHtml = '<span class="path-drawer-status is-default">使用默认</span>';
        inputAttrs = '';
      } else if (isEmpty) {
        clsSuffix = ' is-empty';
        statusHtml = '<span class="path-drawer-status is-missing">缺失</span>';
        inputAttrs = '';
      } else {
        clsSuffix = '';
        statusHtml = '<span class="path-drawer-status is-ok">已配置</span>';
        inputAttrs = '';
      }

      var placeholder = hasFallback ? '默认：' + defaultPath : '输入小程序路径';
      return '<tr>' +
        ...
        '<td><input type="text" class="path-drawer-input' + clsSuffix + '" value="' + escAttr(pathVal) + '" placeholder="' + escHtml(placeholder) + '" data-act-id="' + act.id + '" data-promo-id="' + promoId + '"' + inputAttrs + '></td>' +
        '<td>' + statusHtml + '</td>' +
        '</tr>';
    }).join('');
  ...
  function updatePathRowStatus(inputEl) {
    ...
    if (!statusEl) return;
    var actId = inputEl.getAttribute('data-act-id');
    var thirdActs = (window.MockData && window.MockData.thirdPartyActivities) ? window.MockData.thirdPartyActivities : [];
    var act;
    for (var i = 0; i < thirdActs.length; i++) {
      if (thirdActs[i].id === actId) { act = thirdActs[i]; break; }
    }
    var defaultPath = act ? (act.defaultPath || '') : '';
    var isEmpty = !inputEl.value.trim();
    var hasFallback = isEmpty && act && isIndividualEdit(act) && defaultPath;

    inputEl.classList.remove('is-empty', 'is-default');
    if (!act || !isIndividualEdit(act)) {
      // 统一模式，状态不会在这里变更
    } else if (hasFallback) {
      inputEl.classList.add('is-default');
      statusEl.textContent = '使用默认';
      statusEl.className = 'path-drawer-status is-default';
    } else if (isEmpty) {
  ...
  function updateThirdPartyBadge() {
    ...
    inputs.forEach(function (inp) {
      ...
      if (!isEmpty) return;
      // 如果是独立编辑模式且有默认路径兜底，不算缺失
      var actId = inp.getAttribute('data-act-id');
      var act;
      for (var i = 0; i < thirdActs.length; i++) {
        if (thirdActs[i].id === actId) { act = thirdActs[i]; break; }
      }
      if (act && isIndividualEdit(act) && act.defaultPath) return;
      if (act && !isIndividualEdit(act)) return; // 统一模式不算缺失
      missingCount++;
    });
    if (missingCount > 0) {
  ...
  function getMissingActivities(promoId) {
    ...
    return thirdActs.filter(function (act) {
      var defaultPath = act.defaultPath || '';
      // 有默认路径兜底时不视为缺失（无论是否独立编辑模式）
      if (defaultPath.trim() !== '') return false;
      // 无默认路径，检查该推广位是否已配置独立路径
      var paths = act.promoPaths || {};
